# 演示脚本（约 2 分钟）

初版 Demo 通过开源项目 OpenMinis 在 iOS 的 iSH 环境完成；导入电脑后使用 Codex 继续处理 Django Admin 安全、UI/交互和功能联调。演示重点是可信 AI 问答及其与真实数据库、看板的联动，不将当前页面描述为完整的传统表格式 BI 产品。

## 1. 看板与数据质量

1. 执行 `python manage.py import_sales`，展示重复执行会跳过同一 SHA-256 批次。
2. 打开首页，说明默认显示数据最新 30 天。
3. 切换日期，观察营业额、订单数、客单价、趋势图和 Top 10 同步刷新。
   - 按钮会显示“更新中…”，状态条随后显示实际日期范围；选择数据范围外日期会显示明确空状态，而不是留下空白图表。
4. 展示“今日经营雷达”的近期高点和领先门店。

## 2. 三个可信问题

点击首屏或右下角的“问问经营助手”打开浮窗。演示关闭按钮、`Esc` 和点击遮罩均可关闭；再次打开时此前对话仍然存在。发送问题后观察消息区自动滚到最新问题与“正在查询真实数据…”，回答出现后仍停留在消息区底部，页面本身不会跳动。

### 问题一：哪个品类的门店营业额最高？

真实全量结果：**日料，¥87,292.00**。证据来自 `Sale → Store` JOIN 后按 `store.category` 聚合。

### 问题二：牛肉poke六月卖了多少钱？

真实结果：**¥13,314.00，180 个去重订单**。证据来自 `Sale → Product` JOIN，筛选商品名和 2026-06。

独立核验：

```bash
python manage.py shell -c "from analytics.models import Sale; from django.db.models import Sum; print(Sale.objects.filter(product__name='牛肉poke',date__year=2026,date__month=6).aggregate(Sum('amount')))"
```

### 问题三：客单价最近是涨了还是跌了？

“最近”固定解释为**数据中的最新两个完整自然月**，不是演示当天向前推算，也不是 5 月与 6 月比较：

- 2026 年 6 月：营业额 ¥130,511.00 ÷ 3,711 个去重订单 = **¥35.17**；
- 2026 年 7 月：营业额 ¥149,534.00 ÷ 4,150 个去重订单 = **¥36.03**；
- 结论：客单价**上涨约 ¥0.86**。

页面会同时展示两期客单价和变化值；展开“查看数据依据”可核对查询工具、执行模式、实际区间、数据库结果与请求追踪 ID。

## 3. 追问、联动与拒答

1. 在商品问题后追问“那五月呢？”，展示 SQLite 会话继承商品语境并切换月份。
2. 点击“同步到看板”，日期筛选与图表随证据区间更新。
   - “客单价最近”会同步到 `2026-06-01` 至 `2026-07-31`，关闭问答浮窗并在主看板显示更新状态。
3. 问“明天会下雨吗？”，系统说明数据集不包含天气，而不是编造答案。
4. 断开模型网络后再次提问，页面标记“本地可靠解析”，数据库数字仍保持一致。
5. 查询期间观察消息区、发送按钮和外部入口的加载提示；即使本地回答很快，提示也至少可见 500ms。本作业暂未实现流式输出。
6. 在查询过程中关闭浮窗；回答完成后入口变为“查看新回答”，重新打开即可看到保留的最新消息。
7. 将浏览器高度缩短至约 600px，展开“查看数据依据”；证据在消息区内滚动，底部提问框始终可见。

## 4. 自动化证明

```bash
python manage.py test
```

重点展示：答案数字契约、脏数据审计、非法工具拒绝和网络重试降级测试。
